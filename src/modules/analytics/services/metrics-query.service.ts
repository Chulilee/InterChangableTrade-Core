...
export class MetricsQueryService {
  ...
  async queryMetrics(
  ...
  ): Promise<PaginatedResultDto<AnalyticsMetric>> {
    const {
      page = 1,
      limit = 50,
      metricType,
    ...
    const queryBuilder =
      this.analyticsMetricRepository.createQueryBuilder('metric');
    ...
    if (aggregation) {
      queryBuilder.andWhere('metric.aggregation = :aggregation', {
        aggregation,
      });
    }

    if (assetCodes && assetCodes.length > 0) {
      queryBuilder.andWhere('metric.assetCode IN (:...assetCodes)', {
        assetCodes,
      });
    }
    ...
    if (dimensions && dimensions.length > 0) {
      dimensions.forEach((dim, index) => {
        queryBuilder.andWhere(`metric.dimensions->>:${index} IS NOT NULL`, [
          dim,
        ]);
      });
    }
  ...
  async getTimeSeriesData(
    ...
    queryBuilder
      ...
      .andWhere('metric.timestamp BETWEEN :dateFrom AND :dateTo', {
        dateFrom,
        dateTo,
      });
  ...
  async getAggregatedMetrics(
    ...
    for (const metric of results) {
      ...
      aggregated[metric.metricType].min = Math.min(
        aggregated[metric.metricType].min,
        value,
      );
      aggregated[metric.metricType].max = Math.max(
        aggregated[metric.metricType].max,
        value,
      );
    }
    ...
    const executionTime = Date.now() - startTime;
    this.logger.debug(
      `Aggregated metrics query executed in ${executionTime}ms`,
    );
  ...
  async getDashboardSummary(): Promise<Record<string, any>> {
    ...
    return {
      currentDay: {
        tradeVolume: parseFloat(
          todayMetrics[MetricType.TRADE_VOLUME]?.sum || '0',
        ),
      ...
      previousDay: {
        tradeVolume: parseFloat(
          yesterdayMetrics[MetricType.TRADE_VOLUME]?.sum || '0',
        ),
        tradeCount: parseInt(
          yesterdayMetrics[MetricType.TRADE_COUNT]?.sum || '0',
        ),
        activeUsers: parseInt(
          yesterdayMetrics[MetricType.USER_ACTIVE]?.sum || '0',
        ),
