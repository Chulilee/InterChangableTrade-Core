...
export class AnalyticsService {
  ...
  trackAlertTriggered(rule: AlertRule) {
    ...
    const analytics = this.analyticsRepository.create({
      ...
      processingLatency: this.processingTimes[this.processingTimes.length - 1] || 0,
    });
  ...
  trackUserEngagement(userId: string, notificationId: string, action: 'opened' | 'clicked' | 'dismissed') {
  ...
  async getMetrics(): Promise<ProcessingMetrics> {
    const eventsProcessed = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}events_processed`) || '0');
    const alertsTriggered = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}alerts_triggered`) || '0');
    const alertsThrottled = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}alerts_throttled`) || '0');
    const deduplicated = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}events_deduplicated`) || '0');
    const errors = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}processing_errors`) || '0');
  ...
  async getChannelPerformance(): Promise<ChannelPerformance[]> {
    const channels = ['email', 'sms', 'websocket', 'mobile_push', 'telegram', 'discord'];
    const results: ChannelPerformance[] = [];

    for (const channel of channels) {
      const attempted = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}channel:${channel}:attempted`) || '0');
      const delivered = parseInt(await this.redis.get(`${this.METRICS_KEY_PREFIX}channel:${channel}:delivered`) || '0');
      const failed = attempted - delivered;
      
      // Get average latency
      const latencies = await this.redis.lrange(`${this.METRICS_KEY_PREFIX}channel:${channel}:latencies`, 0, -1);
      const avgLatency = latencies.length > 0
        ? latencies.reduce((sum, latency) => sum + parseInt(latency), 0) / latencies.length
        : 0;
  ...
  async getMostValuableAlertTypes(limit: number = 10): Promise<any[]> {
    ...
    const analytics = await this.analyticsRepository
      ...
      .addSelect('SUM(CASE WHEN analytics.userAction = \'clicked\' THEN 1 ELSE 0 END)', 'clickCount')
    ...
    return analytics.map(row => ({
      ...
      ctr: parseInt(row.triggerCount) > 0 ? parseInt(row.clickCount) / parseInt(row.triggerCount) : 0,
    }));
  }

  private async persistMetrics() {
    // Daily persistence of aggregated metrics
    if (Math.random() < 0.01) { // Only persist ~1% of the time to avoid overhead
