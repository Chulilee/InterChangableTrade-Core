...
import { AlertRule, AlertCondition, AlertAction } from '../interfaces/alert-rule.interface';
...
export class AlertingService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.error(`❌ Failed to subscribe to Redis channel: ${this.eventChannel}`, error);
    }
  }

  private async handleEvent(event: BlockchainEvent) {
    const startTime = Date.now();
    this.logger.debug(`📥 Received event: ${event.eventType} [${event.uniqueId}]`);
    ...
    try {
      ...
      const matchingRules = await this.ruleEvaluationService.evaluateRules(event);
      ...
      this.analyticsService.trackEventProcessed(startTime, matchingRules.length);
    } catch (error) {
  ...
  private async sendAlert(rule: AlertRule, event: BlockchainEvent) {
    ...
    const recipients = await this.ruleEvaluationService.getAffectedUsers(event, rule);
    
    for (const userId of recipients) {
      await this.notificationsService.sendNotificationToUser(userId, notification, rule.metadata);
    }
    
    this.logger.log(`🚀 Alert sent: "${rule.name}" to ${recipients.length} users`);
  }
}