...
export class StellarGatewayController {
  ...
  @ApiOperation({
    summary: 'Fetch account summary and balances from Stellar network',
  })
  @ApiHeader({
    name: 'X-Client-ID',
  ...
  @Post('settle')
  @ApiOperation({
    summary: 'Execute a settlement transaction between two accounts',
  })
  @ApiHeader({
    name: 'X-Client-ID',
    description: 'Client identifier for rate limiting',
  ...
  @ApiResponse({
    status: 200,
    description: 'Settlement transaction created successfully',
  })
  @ApiResponse({ status: 429, description: 'Rate limit exceeded' })
  async executeSettlement(
    ...
    const clientId = this.extractClientId(headers);
    const response = await this.apiGateway.executeSettlement(
      settlementRequest,
      clientId,
    );
  ...
  @Post('transactions')
  @ApiOperation({
    summary: 'Submit a signed transaction to the Stellar network',
  })
  @ApiHeader({
    name: 'X-Client-ID',
    description: 'Client identifier for rate limiting',
  ...
  async submitTransaction(
    ...
    const clientId = this.extractClientId(headers);
    const response = await this.apiGateway.submitTransaction(
      submitRequest.transactionXdr,
      clientId,
    );
