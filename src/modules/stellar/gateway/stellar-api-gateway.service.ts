...
import { Networks, TransactionBuilder, Operation, Asset, Horizon, xdr } from '@stellar/stellar-sdk';
import { StellarConnectionPoolService, PooledConnection } from './stellar-connection-pool.service';
...
export class StellarApiGatewayService {
  ...
  private async executeWithMonitoring<T>(
    ...
    try {
      ...
      this.logger.error(`Request ${requestId} failed after ${latency}ms: ${errorMessage}`);
  ...
  async getAccount(accountId: string, clientId: string): Promise<GatewayResponse<AccountSummary>> {
    ...
    return this.requestQueue.enqueue(clientId, async () => {
      ...
      try {
        ...
        this.logger.debug(`Executing getAccount for ${accountId} with connection ${connection.id}`);
  ...
  async executeSettlement(
    ...
    return this.requestQueue.enqueue(clientId, async () => {
      ...
      try {
        connection = await this.connectionPool.acquireConnection();
        this.logger.debug(`Executing settlement for ${settlementRequest.fromAccount} -> ${settlementRequest.toAccount} with connection ${connection.id}`);
        ...
        this.logger.log(`Settlement transaction ${mockTxHash} created for trade`);

        return this.executeWithMonitoring(requestId, clientId, async () => mockTxHash);
      } catch (error) {
        if (connection) {
          this.connectionPool.releaseConnection(connection.id);
        }
  ...
  async getTransaction(
    ...
    return this.requestQueue.enqueue(clientId, async () => {
      ...
      try {
        connection = await this.connectionPool.acquireConnection();
        this.logger.debug(`Fetching transaction ${transactionHash} with connection ${connection.id}`);
        
        const transaction = await connection.server.transactions().transaction(transactionHash).call();
        this.connectionPool.releaseConnection(connection.id);

        return this.executeWithMonitoring(requestId, clientId, async () => transaction);
      } catch (error) {
        if (connection) {
          this.connectionPool.releaseConnection(connection.id);
        }
  ...
  async submitTransaction(
    ...
    return this.requestQueue.enqueue(clientId, async () => {
      ...
      try {
        connection = await this.connectionPool.acquireConnection();
        this.logger.debug(`Submitting transaction with connection ${connection.id}`);
        
        // In this Stellar SDK version, submitTransaction accepts XDR strings as well
        const result = await connection.server.submitTransaction(transactionXdr as any);
        this.connectionPool.releaseConnection(connection.id);

        return this.executeWithMonitoring(requestId, clientId, async () => result);
      } catch (error) {
        if (connection) {
          this.connectionPool.releaseConnection(connection.id);
        }
